...
const controller = () => {
  ...
  components.genPath.addEventListener("click", async () => {
    ...
    const url = baseUrl + `?origem=Timisoara&destino=Bucareste`;
    console.log(await fetchPathFinder(url));
  });

  components.radioSim.addEventListener("change", () => {
    if (components.radioSim.checked) inputFile.style.display = "none";
    main.innerHTML = "";
  });

  components.radioNao.addEventListener("change", () => {
    if (components.radioNao.checked) inputFile.style.display = "block";
    main.innerHTML = "";
  });
